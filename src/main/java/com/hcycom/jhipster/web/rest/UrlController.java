...
import com.hcycom.jhipster.service.GetwayService;
...
public class UrlController {
	...
	@RequestMapping(value = "/updateUrlMapping", method = RequestMethod.GET)
	@Timed
	@ApiOperation(value = "更新或者获取所有接口", notes = "更新或者获取所有应用接口,无权限管理", httpMethod = "GET")
	public void getUrlMapping() {
		List<Map<String, Object>> serverjson=getwayService.getGateway();
		List<String> list=new ArrayList<String>();
		for (Map<String, Object> map2 : serverjson) {
			String serviceId=map2.get("serviceId").toString();
			list.add(serviceId);
		}
		list.forEach(service->{
			String json="";
			if(service.equals("jhipstergateway")){
				json=getwayService.getgatewayDocumentation();
			}else{
				json = getwayService.getDocumentation(service);
			}
			List<Map<String, String>> map = getinterface(json,service);
			List<Authority> Interfacelist = authorityMapper.getAllAuthorityByAuthority_type("4");
			...
			for (Map<String, String> map2 : map) {
				...
				int k = 0;
				for (Authority authority2 : Interfacelist) {
	...
	private List<Map<String, String>> getinterface(String urljson,String server) {
		...
		for (Map<String, String> pathsmap : pathslist) {
			for (String urlkey : pathsmap.keySet()) {
				...
				for (String methodkey : methodmap.keySet()) {
					urlmap.put("url", server + urlkey + "--" + methodkey);
